...
- Currency Strength Meter : **se démarquer de PMT** — vert de marque `#00e676` / `rgba(0,230,118,…)`, rouge `#ff3d00` / `rgba(255,61,0,…)` (les valeurs PMT EXACTES `rgba(0,218,80)`/`rgba(255,0,0)` ont été **retirées partout**). Garder l'**ergonomie** (égaliseur bidirectionnel) mais un **visuel distinct**.
...
- **Smart Bias Tracker** (onglet BIAS — haute densité, architecture bidirectionnelle ; **ergonomie inspirée de PMT mais APPARENCE à différencier**) :
  ...
  - **Header du panneau (haut-droite)** : 2 dropdowns compacts = **sélecteur de devise active** (avec micro-drapeau) + **sélecteur d'historique de dates** au format `1-7/06/2026` (**à faire évoluer vers un format distinct de PMT**, ex. `1–7 juin 2026`).
