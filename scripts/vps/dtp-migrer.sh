...
case "$(uname -s)" in
  Linux|Darwin)
    ...
    # On referme les DEUX connexions maitresses : le script parle a la cible ET a l'ancien
    # serveur. En oublier une laisserait un processus ssh vivant jusqu'a expiration.
    trap 'for h in "$CIBLE" "$ANCIEN"; do ssh -O exit -o ControlPath="$MUXDIR/%r@%h:%p" root@"$h" 2>/dev/null; done; rm -rf "$MUXDIR"' EXIT
    ;;
