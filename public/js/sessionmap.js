...
(function () {
  ...
  window.buildSessionMap = function buildSessionMapLeaflet() {
    ...
    // Tuiles SATELLITE (Esri World Imagery — sans clé ; attribution requise)
    L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', {
      maxZoom: 18, attribution: 'Tuiles &copy; Esri', crossOrigin: true
    }).addTo(map);
    // Étiquettes discrètes (frontières / villes) par-dessus le satellite
    L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/Reference/World_Boundaries_and_Places/MapServer/tile/{z}/{y}/{x}', {
      maxZoom: 18, opacity: 0.5, crossOrigin: true
    }).addTo(map);

    // Terminateur jour/nuit (si le plugin a chargé)
    if (typeof L.terminator === 'function') {
      try {
        var term = L.terminator({ fillColor: '#04060d', fillOpacity: 0.5, color: '#0a1020', weight: 0, interactive: false });
        term.addTo(map);
